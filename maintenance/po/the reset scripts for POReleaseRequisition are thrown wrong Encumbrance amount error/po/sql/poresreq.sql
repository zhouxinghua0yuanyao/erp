REM dbdrv: none
 ...
 /* $Header: poresreq.sql 120.3.12020000.3 2016/02/27 01:16:38 pla ship $ */
 ...
 BEGIN
  ...
  WHERE  EXISTS (SELECT 'Wrong Encumbrance Amount'
			...
			AND d.set_of_books_id = g.ledger_id AND g.currency_code = c.currency_code	--BUG 19254890
                        ...
                        AND Nvl(d.encumbered_amount, 0) <>
                             DECODE (c.minimum_accountable_unit,
                                     NULL, 
                                     DECODE (c.precision,       
                                             NULL, 
                                             l.unit_price * d.req_line_quantity  + Nvl(d.nonrecoverable_tax, 0),                                                 
                                             Round( (l.unit_price * d.req_line_quantity)  + Nvl(d.nonrecoverable_tax, 0), 
                                                    c.precision)
                                            ),
                                     Round( ((l.unit_price * d.req_line_quantity  + Nvl(d.nonrecoverable_tax, 0))/c.minimum_accountable_unit) * c.MINIMUM_ACCOUNTABLE_UNIT )
                                    )
                 UNION
			...
			AND d.set_of_books_id = g.ledger_id AND g.currency_code = c.currency_code	--BUG 19254890
                        ...
                        AND Nvl(d.encumbered_amount, 0)  <>
                             DECODE (c.minimum_accountable_unit,
                                     ...
                                     DECODE (c.precision,
                                             NULL,
                                             d.req_line_amount + Nvl(d.nonrecoverable_tax, 0),
                                             Round(  d.req_line_amount + Nvl(d.nonrecoverable_tax, 0),
                                                    c.precision)
                                            ),
                                     Round( (( d.req_line_amount  + Nvl(d.nonrecoverable_tax, 0))/c.minimum_accountable_unit) * c.MINIMUM_ACCOUNTABLE_UNIT )    
                                    ) );	--BUG 21531821 
 EXCEPTION
 when NO_DATA_FOUND THEN
   NULL;
