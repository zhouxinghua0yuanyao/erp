#
# $Header: install.pl 121.3 2011/01/22 00:00:00 Zheng.liu noship $
#
# *===========================================================================+
...
my $au_top; 	# cux top path
my $au_top2; 	# au top path
...
sub processInstallStep
{
	...
	if(! -d $sourceFiledir){
		...
		printLogAndOut("source file director not found, skip this step\n",1);
		return;
	};
...
sub copySourceToStage
{
	...
	if($_stepConfig{'multilanguage'} eq "Y"){
		for(my $iLang=0; $iLang<@_lang; $iLang++){
			$sourceFileLangdir = getOSfilepath($sourceFiledir."/".$_lang[$iLang]);
			$stageFileLangdir = getOSfilepath($stageFiledir."/".$_lang[$iLang]);
			# skip not current language director
			next if(! -d $sourceFileLangdir);
			
			if(!-d $stageFileLangdir){
				if(!createDir($stageFileLangdir)){ errorAndExit("Could not create directory: $stageFileLangdir\n");}
			}
			copyFileAndDir($sourceFileLangdir, $stageFileLangdir, $_stepConfig{'filter'}, "");
		}
	}else{
...
sub runFndXdfcmp
{
	...
	foreach $entry (@fileList){
		...
		if($xdfInfo{'objectSchemaName'} eq "" || $xdfInfo{'objectName'} eq "" ||$xdfInfo{'objectType'} eq "" ){
			printLogAndOut("Error : can not get xdf information\n", 1);
			next;
		}
		...
		# set schema_Pwd 
		if ($xdfInfo{'objectSchemaName'} eq $arg{'dbschema'} ){
			$schemaPassword = $arg{'dbschemapwd'};
	  }else{
	  	$schemaPassword = $xdfInfo{'objectSchemaName'} ;
		}
		$cmpScript = "adjava oracle.apps.fnd.odf2.FndXdfCmp";
		$cmpScript .= " $xdfInfo{'objectSchemaName'} $schemaPassword";
		$cmpScript .= " $arg{'appsusr'} $arg{'appspwd'}";
		$cmpScript .= " thin $ctx{'s_dbhost'}.$ctx{'s_dbdomain'}:$ctx{s_dbport}:$ctx{s_dbSid}$rac_mode";
...
sub runF60gen
{
	...
	foreach $entry (@fileList){
		...
		$cmdScript = "f60gen Module=$fileName";
		...
		if($moduleType eq "LIBRARY"){
			chdir(getOSfilepath("$au_top2/resource"));
		}elsif($moduleType eq "FORM"){
			chdir(getOSfilepath("$au_top2/forms/$currentLang"));
		}
		$err = runSystemCmd($cmdScript);
  	chdir($arg{'currentdir'});
		#$err = $perlOps->runPipedCmd($cmdScript,$arg{'appspwd'});
		if($err){
			printLogAndOut("Error occur when run script\n", 1);
		}
...
sub runFrmcmpBatch
{
	...
	foreach $entry (@fileList){
		...
		$cmdScript = "frmcmp_batch Module=$fileName";
		$cmdScript .= " Userid=$arg{'appsusr'}/$arg{'appspwd'}";
		$cmdScript .= " Module_Type=$moduleType";
		$cmdScript .= " Output_File=$outFile";
		$cmdScript .= " > $logFile";
...
sub runFndload
{
	...
	foreach $entry (@fileList){
		...
		if($ldtInfo{'LDRCONFIG'} eq "" || $ldtInfo{'LANGUAGE'} eq "" ){
			printLogAndOut("Error : can not get ldt file information\n", 1);
			next;
		}
...
sub runXdoload
{
	...
	foreach $entry (@fileList){
		...
		if($_stepConfig{'xdolobtype'} eq "TEMPLATE" && $fileExt eq ""){
			printLogAndOut("Error can not get file type: $fileName\n",1);
			next;
		}
		...
		$loadScript .= " -JDBC_CONNECTION $ctx{'s_dbhost'}.$ctx{'s_dbdomain'}:$ctx{s_dbport}:$ctx{s_dbSid}$rac_mode";
...
sub runXmlImport
{
	...
	foreach $entry (@fileList){
		...
		$loadScript .= " -dbconnection \"(description = (address_list = (address = (community = tcp.world)(protocol = tcp)(host = $ctx{'s_dbhost'}.$ctx{'s_dbdomain'})(port = $ctx{s_dbport})))(connect_data = (sid = $ctx{s_dbSid}$rac_mode)))\"";
		$loadScript .= " > $logFile";
		printLogAndOut("$loadScript\n", 1);
...
sub runCPShell
{
	...
	foreach $entry (@fileList){
		...
		if ($arg{'osname'} eq "win32"){
			...
			# $err = $perlOps->runPipedCmd($loadScript,$arg{'appspwd'});
			if($err){
				printLogAndOut("Error link file $fndcpesrLink.\n", 1);
			}
...
sub runProgram
{
	if($_stepConfig{'executeprogram'} eq "sqlplus"){
		...
		runF60gen($currentDir, $_stepConfig{'filter'});
