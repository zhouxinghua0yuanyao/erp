...
my $alr_top;	# alr top path
my $xdo_top;  # xdo top path
my $rac_mode; # rac mode
my $bne_top;  #bne top path
...
sub main{
  ...
  ## Verifying environment settings $APPL_TOP, $NLS_LANG, $FND_TOP, $AU_TOP, $CONTEXT_FILE
	if ( ("$ENV{APPL_TOP}x" eq "x") ||
	     ...
	     ("$ENV{FND_TOP}x" eq "x") ||
	     ("$ENV{CUX_TOP}x" eq "x") ||
	     ("$ENV{CONTEXT_FILE}x" eq "x") || ("$ENV{BNE_TOP}x" eq "x")) {
...
sub getSqlScript
{
    ...
    $sqlScript .= " ORDER BY INSTALLED_FLAG, LANGUAGE_CODE;\n";
	  ...
	  $sqlScript .= "EXIT;\n";	  
		...
		$sqlScript .= "and type=decode('$object_type','PACKAGE_SPECIAL','PACKAGE','PACKAGE_BODY','PACKAGE BODY','TRIGGER','TRIGGER','TYPE_SPECIAL','TYPE','TYPE_BODY','TYPE BODY')\n";
...
sub downloadObjects
{
	...
	if($_typeConfig{'executeprogram'} ne ""){
		executeProgram();
	}
...
sub runProgram
{
	if($_typeConfig{'executeprogram'} eq "FndXdfGen"){
		runFndXdfGen();
	}elsif($_typeConfig{'executeprogram'} eq "XDOLoader"){
		runXDOLoader();
	}elsif($_typeConfig{'executeprogram'} eq "FNDLOAD"){
		runFNDLOAD();
	}elsif($_typeConfig{'executeprogram'} eq "getobjects"){
		getPlsqlObjects();
	}elsif($_typeConfig{'executeprogram'} eq "CopySource"){
		copySourceCode();
	}
}
...
sub runFNDLOAD
{
  ...
  my @logFiles;
    ...
    if ( $object eq "" ){
		...
		if (defined($_typeConfig{'needapplname'}) && $_typeConfig{'needapplname'} eq "Y" && $objectowner eq "" ) {
			printLogAndOut("Error : fndload object owner is null\n", 1);
			next;
	  }
	  
	  if (defined($_typeConfig{'needapplname'}) && $_typeConfig{'needapplname'} eq "N" ){
	  	if (defined($_typeConfig{'defaultapplname'})){
	  		$objectowner = $_typeConfig{'defaultapplname'};
	  	}
	  }
    
    if ($currentType eq "ALERT") {
		  $lctFile = getOSfilepath($_typeConfig{'ctlfile'});
		}else { 
			$lctFile = getOSfilepath("$fnd_top/patch/115/import/$_typeConfig{'ctlfile'}.lct");
		}
	
    if ($currentType eq "INTEGRATOR") {
		  $lctFile = getOSfilepath($_typeConfig{'ctlfile'});
		}	
		...
		$loadScript .= " $lctFile";
		$loadScript .= " $ldtFile";
		$loadScript .= " $_typeConfig{'fndloaddatatype'}";
		$loadScript .= " $_typeConfig{'fndloadparaname'}=$object";
		
		if (defined($_typeConfig{'needapplname'}) && $_typeConfig{'needapplname'} eq "Y" ) {
			$loadScript .= " $_typeConfig{'applparaname'}=$objectowner";
		}
  ...
  my @logFiles;
	...
	for(my $iobject=0; $iobject<@_objectList; $iobject++){
	  ...
	  if ($objectowner eq "" || $object eq "") {
		...
		$lctFile = getOSfilepath(eval("\"".$_typeConfig{'ctlfile'}."\"")) ;
